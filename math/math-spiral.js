...
async function loadServerProgress(options={}){
  ...
  progressLoadPromise=(async()=>{
    try{
      ...
      state.progressError=error instanceof Error?error:new Error(String(error));
...
function qRounding(){const n=rand(101,999),ans=Math.round(n/10)*10,options=[ans,ans-10,ans+10,ans+20];return baseQuestion('반올림하기',`${n}을 십의 자리까지 반올림하면?`,`<div class="round-number">${n}</div>`,options,ans,'일의 자리 숫자가 5 이상인지 살펴보세요.',`반올림한 값은 ${ans}입니다.`);}
function qFractionOp(){const den=[4,5,6,8][rand(0,3)],a=rand(1,den-2),b=rand(1,den-a),ans=`${a+b}/${den}`,choices=[ans,`${a+b}/${den*2}`,`${a*b}/${den}`,`${a+b}/${den+1}`,`${Math.max(1,a+b-1)}/${den}`],options=[...new Set(choices)].slice(0,3);return baseQuestion('같은 분모의 분수 덧셈',`${a}/${den}+${b}/${den}=?`,fractionBar(a+b,den),options,ans,'분모는 그대로 두고 분자를 더하세요.',`답은 ${ans}입니다.`);}
...
function qLineGraph(){const vals=[rand(2,5)];for(let i=1;i<4;i++)vals.push(Math.max(1,vals[i-1]+rand(-1,3)));const trend=vals[3]>vals[0]?'up':vals[3]<vals[0]?'down':'same',explain=trend==='up'?'전체적으로 증가했습니다.':trend==='down'?'전체적으로 감소했습니다.':'처음과 마지막 값이 같습니다.';return baseQuestion('꺾은선그래프 해석','처음과 마지막을 비교하면 전체적으로 어떻게 변했나요?',lineVisual(vals),[{value:'up',label:'증가했다'},{value:'down',label:'감소했다'},{value:'same',label:'변함없다'}],trend,'첫 점과 마지막 점의 높이를 비교하세요.',explain);}
