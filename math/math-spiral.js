...
const STEP_KEYS = ['experience', 'manipulation', 'visualization', 'symbolization', 'explanation', 'application'];
...
function curriculumLessons(){return CURRICULUM.flatMap(area=>BANDS.flatMap(band=>area.bands[band].map(item=>({...item,standards:[...item.standards],domainId:area.id,domainTitle:area.title,band}))));}
function completedPercent(){const total=allLessons().length;return total?Math.round(state.completed.size/total*100):0;}
...
window.getAiedueMathCurriculumLessons=function getAiedueMathCurriculumLessons(){return curriculumLessons();};
window.getAiedueMathProgress=function getAiedueMathProgress(){return{loaded:state.progressLoaded,loading:state.progressLoading,error:state.progressError?.message||'',completedNodeIds:[...state.completed]};};

function completedNodeIdsFrom(payload){
  if(Array.isArray(payload?.completedNodeIds))return payload.completedNodeIds;
  const docs=Array.isArray(payload?.progressDocs)?payload.progressDocs:[];
  return docs.filter(progress=>progress?.completed===true||['completed','mastered','basic','stable','extended'].includes(String(progress?.masteryState||progress?.status||'').toLowerCase())).map(progress=>progress.nodeId||progress.lessonId||progress.id);
}

function notifyProgressChanged(source){
  window.dispatchEvent(new CustomEvent('aiedue-math-progress-changed',{detail:{source,loaded:state.progressLoaded,completedNodeIds:[...state.completed]}}));
}

function replaceCompletedProgress(payload,source='server'){
  const knownIds=new Set(allLessons().map(item=>item.id));
  state.completed=new Set(completedNodeIdsFrom(payload).map(String).filter(id=>knownIds.has(id)));
  state.progressLoaded=true;
  state.progressError=null;
  notifyProgressChanged(source);
}

async function loadServerProgress(options={}){
  if(progressLoadPromise){
    if(options.force){await progressLoadPromise;return loadServerProgress();}
    return progressLoadPromise;
  }
  const api=window.aiedueMathData;
  if(!api||typeof api.loadProgress!=='function')return false;
  state.progressLoading=true;
  progressLoadPromise=(async()=>{
    try{
      if(typeof api.ready==='function')await api.ready();
      const result=await api.loadProgress();
      replaceCompletedProgress(result,'server-load');
      return true;
    }catch(error){
      const normalizedError=error instanceof Error?error:new Error(String(error));
      if(/먼저 로그인/.test(normalizedError.message)){state.progressError=null;return false;}
      state.progressError=normalizedError;
...
window.refreshAiedueMathProgress=loadServerProgress;
window.addEventListener('aiedue-math-data-ready',(event)=>{
  if(Array.isArray(event.detail?.completedNodeIds))replaceCompletedProgress(event.detail,'data-ready');
  void loadServerProgress();
});
window.addEventListener('aiedue-math-progress-updated',(event)=>{
  if(Array.isArray(event.detail?.completedNodeIds)||Array.isArray(event.detail?.progressDocs))replaceCompletedProgress(event.detail,'server-update');
});

function startProgressSync(){void loadServerProgress();}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',startProgressSync,{once:true});
else startProgressSync();

function showSection(id){document.querySelectorAll('.view-section').forEach(section=>{const visible=section.id===id;section.classList.toggle('hidden',!visible);section.style.display=visible?'flex':'none';});window.collapseRpgHud?.();document.getElementById('aiedue-rpg-hud')?.classList.remove('hidden');}
window.openSpiralMap=function(){showSection('spiral-map-section');renderMap();};
window.openSpiralCurriculum=window.openSpiralMap;
window.openSpiralLesson=function(id,options={}){const found=findLesson(id);if(!found)return false;state.current=found;state.band=found.band;state.step=0;state.attemptSource=options?.attemptSource==='review'?'review':'normal';state.assignmentId=typeof options?.assignmentId==='string'?options.assignmentId:null;state.question=createTrackedQuestion(found.item,0);showSection('spiral-lesson-section');renderLesson();return true;};
...
function createTrackedQuestion(item,step){questionSequence+=1;return{...makeQuestion(item.type,step),questionId:`${item.id}-${step}-${Date.now().toString(36)}-${questionSequence.toString(36)}`,startedAt:Date.now()};}
function renderLesson(){const root=document.getElementById('spiral-lesson-content');if(!root||!state.current)return;const{area,band,item}=state.current;if(!state.question)state.question=createTrackedQuestion(item,state.step);root.innerHTML=`<main class="spiral-shell lesson-shell"><header class="spiral-lesson-head spiral-panel" style="--area:${area.color}"><button class="spiral-back" onclick="openSpiralMap()">← 배움 지도</button><div class="lesson-heading"><span>${area.icon}</span><div><p>${area.title} · ${band.replace('-','~')}학년</p><h1>${item.title}</h1><p>${item.summary}</p></div></div><div class="standard-chips">${item.standards.map(s=>`<span>${s}</span>`).join('')}</div></header><section class="spiral-panel spiral-learning"><nav class="spiral-step-list" aria-label="학습 단계">${STEPS.map((s,i)=>`<button class="spiral-step ${i===state.step?'active':''} ${i<state.step?'done':''}" data-step="${i}"><b>${i+1}</b>${s}</button>`).join('')}</nav><div class="spiral-task" id="spiral-task">${renderTask(item,state.question)}</div></section></main>`;root.querySelectorAll('.spiral-step').forEach(btn=>btn.addEventListener('click',()=>{const next=Number(btn.dataset.step);if(!state.submitting&&next<=state.step){state.step=next;state.question=createTrackedQuestion(item,next);renderLesson();}}));bindTaskEvents(item);}
...
async function checkAnswer(item,button){
  ...
  const correct=String(question.answer)===value;
  state.submitting=true;
  setTaskBusy(true);
  feedback('답안을 저장하고 있어요.','hint');
  try{
    await recordServerAttempt(attemptPayload(item,question,value,correct));
  }catch(error){
    console.error('Math attempt save failed:',error);
    if(state.question===question){feedback('답을 저장하지 못했어요. 연결을 확인한 뒤 다시 선택해 주세요.','bad');setTaskBusy(false);}
    state.submitting=false;
    return;
  }
  if(state.question!==question){state.submitting=false;return;}
  button.classList.add(correct?'correct':'wrong');
  if(!correct){
    document.querySelector(`.spiral-option[data-value="${CSS.escape(String(question.answer))}"]`)?.classList.add('correct');
    feedback(`다시 살펴봐요. ${question.explain}`,'bad');
    state.submitting=false;
    setTimeout(()=>{state.question=createTrackedQuestion(item,state.step);renderLesson();},1600);
    return;
  }
  feedback(`잘했어요! ${question.explain}`,'good');
  if(state.step<STEPS.length-1){
    state.submitting=false;
    setTimeout(()=>{state.step+=1;state.question=createTrackedQuestion(item,state.step);renderLesson();},1000);
    return;
  }
  state.completed.add(item.id);
  state.submitting=false;
  notifyProgressChanged('lesson-completed');
  void loadServerProgress({force:true});
  setTimeout(()=>{feedback('개념을 완료했어요! 배움 지도에서 다음 개념이 열렸습니다.','good');const actions=document.querySelector('.spiral-actions');if(actions)actions.innerHTML='<button class="spiral-action" onclick="openSpiralMap()">배움 지도로 돌아가기</button><button class="spiral-action secondary" id="spiral-again">다시 연습</button>';document.getElementById('spiral-again')?.addEventListener('click',()=>{state.step=0;state.question=createTrackedQuestion(item,0);renderLesson();});},700);
}
...
function baseQuestion(title,prompt,visual,options,answer,hint,explain){const normalized=options.map(value=>typeof value==='object'?value:({value,label:value}));const unique=[...new Map(normalized.map(option=>[String(option.value),option])).values()];if(!unique.some(option=>String(option.value)===String(answer)))unique.unshift({value:answer,label:answer});return{title,prompt,visual,options:shuffle(unique),answer,hint,explain};}
...
function qEstimate(){const a=rand(21,78),b=rand(12,49),sum=a+b,rounded=Math.round(sum/10)*10;return baseQuestion('계산 결과 어림하기',`${a}+${b}의 정확한 합을 구한 뒤, 가장 가까운 십의 자리 수를 고르세요.`,`<div class="number-line-simple"><span>${rounded-20}</span><span>${rounded-10}</span><span>${rounded}</span><span>${rounded+10}</span></div>`,[rounded-10,rounded,rounded+10,rounded+20],rounded,`${a}+${b}를 계산한 뒤 합의 일의 자리가 5 이상인지 살펴보세요.`,`정확한 합은 ${sum}이고, ${sum}을 십의 자리까지 반올림하면 ${rounded}입니다.`);}
...
function qRounding(step){let n=rand(101,999);const mode=step%3;if(mode===1){const ans=Math.floor(n/10)*10;return baseQuestion('버림하기',`${n}을 십의 자리까지 버림하면?`,`<div class="round-number">${n}</div>`,[ans,ans+10,ans-10,ans+20],ans,'일의 자리 수를 0으로 바꾸고 더하지 않습니다.',`십의 자리까지 버림한 값은 ${ans}입니다.`);}if(mode===2){if(n%10===0)n+=1;const ans=Math.ceil(n/10)*10;return baseQuestion('올림하기',`${n}을 십의 자리까지 올림하면?`,`<div class="round-number">${n}</div>`,[ans,ans-10,ans+10,ans-20],ans,'일의 자리에 0이 아닌 수가 있으므로 다음 십의 자리 수로 올립니다.',`십의 자리까지 올림한 값은 ${ans}입니다.`);}const ans=Math.round(n/10)*10;return baseQuestion('반올림하기',`${n}을 십의 자리까지 반올림하면?`,`<div class="round-number">${n}</div>`,[ans,ans-10,ans+10,ans+20],ans,'일의 자리 숫자가 5 이상인지 살펴보세요.',`십의 자리까지 반올림한 값은 ${ans}입니다.`);}
function qFractionOp(step){const den=[4,5,6,8][rand(0,3)],mode=step%4;if(mode===1){const a=rand(2,den-1),b=rand(1,a-1),ans=`${a-b}/${den}`;return baseQuestion('같은 분모의 분수 뺄셈',`${a}/${den}−${b}/${den}=?`,fractionBar(a-b,den),[ans,`${a+b}/${den}`,`${a-b}/${den*2}`,`${Math.max(1,a-b+1)}/${den}`],ans,'분모는 그대로 두고 분자를 빼세요.',`답은 ${ans}입니다.`);}if(mode===2){const a=rand(2,3),b=rand(1,Math.max(1,Math.floor((den-1)/a))),product=a*b,ans=`${product}/${den}`;return baseQuestion('자연수와 분수의 곱셈',`${a}×${b}/${den}=?`,fractionBar(product,den),[ans,`${a+b}/${den}`,`${product}/${den*a}`,`${b}/${den}`],ans,`${b}/${den}을 ${a}번 더한다고 생각해 보세요.`,`분자를 ${a}배 하여 ${ans}입니다.`);}if(mode===3){const a=rand(2,den-1),ans=String(a);return baseQuestion('분수의 나눗셈',`${a}/${den}÷1/${den}=?`,`<div class="equation-visual">${a}/${den} ÷ 1/${den}</div>`,nums(a),ans,`1/${den}이 ${a}/${den} 안에 몇 개 들어 있는지 생각해 보세요.`,`1/${den}이 ${a}개 있으므로 답은 ${a}입니다.`);}const a=rand(1,den-2),b=rand(1,den-a),ans=`${a+b}/${den}`;return baseQuestion('같은 분모의 분수 덧셈',`${a}/${den}+${b}/${den}=?`,fractionBar(a+b,den),[ans,`${a+b}/${den*2}`,`${a*b}/${den}`,`${Math.max(1,a+b-1)}/${den}`],ans,'분모는 그대로 두고 분자를 더하세요.',`답은 ${ans}입니다.`);}
function qDecimalOp(step){const mode=step%4;if(mode===1){const a=rand(20,69)/10,b=rand(2,Math.floor(a*10)-1)/10,ans=(a-b).toFixed(1);return baseQuestion('소수 뺄셈',`${a.toFixed(1)}−${b.toFixed(1)}=?`,`<div class="decimal-grid">${a.toFixed(1)} − ${b.toFixed(1)}</div>`,nums(Math.round((a-b)*10),4).map(x=>({value:(x/10).toFixed(1),label:(x/10).toFixed(1)})),ans,'소수점을 맞추어 빼세요.',`답은 ${ans}입니다.`);}if(mode===2){const a=rand(11,39)/10,b=rand(2,6),ans=(a*b).toFixed(1);return baseQuestion('소수 곱셈',`${a.toFixed(1)}×${b}=?`,`<div class="decimal-grid">${a.toFixed(1)} × ${b}</div>`,nums(Math.round(a*b*10),8).map(x=>({value:(x/10).toFixed(1),label:(x/10).toFixed(1)})),ans,`${a.toFixed(1)}을 ${b}번 더한다고 생각해 보세요.`,`답은 ${ans}입니다.`);}if(mode===3){const divisor=rand(2,5),quotient=rand(11,39)/10,dividend=divisor*quotient,ans=quotient.toFixed(1);return baseQuestion('소수 나눗셈',`${dividend.toFixed(1)}÷${divisor}=?`,`<div class="decimal-grid">${dividend.toFixed(1)} ÷ ${divisor}</div>`,nums(Math.round(quotient*10),6).map(x=>({value:(x/10).toFixed(1),label:(x/10).toFixed(1)})),ans,`${divisor}×?=${dividend.toFixed(1)}인 수를 찾으세요.`,`답은 ${ans}입니다.`);}const a=rand(11,49)/10,b=rand(2,9)/10,ans=(a+b).toFixed(1);return baseQuestion('소수 덧셈',`${a.toFixed(1)}+${b.toFixed(1)}=?`,`<div class="decimal-grid">${a.toFixed(1)} + ${b.toFixed(1)}</div>`,nums(Math.round((a+b)*10),4).map(x=>({value:(x/10).toFixed(1),label:(x/10).toFixed(1)})),ans,'소수점을 맞추어 계산하세요.',`답은 ${ans}입니다.`);}
...
function qRatio(){const a=rand(1,4),b=rand(2,6),ans=`${a}:${b}`,choices=[ans,`${b}:${a}`,`${a+b}:${b}`,`${a}:${a+b}`],options=[...new Set(choices)].slice(0,3);return baseQuestion('비로 나타내기',`빨간 구슬 ${a}개와 파란 구슬 ${b}개의 비는?`,`<div class="beads"><span>${'🔴'.repeat(a)}</span><span>${'🔵'.repeat(b)}</span></div>`,options,ans,'말한 순서대로 두 수를 : 기호로 연결하세요.',`빨강:파랑=${ans}입니다.`);}
function qPercent(){const n=[10,20,25,50,75][rand(0,4)],ans=`${n}%`,choices=[ans,`${100-n}%`,`${n/100}%`,`${Math.min(100,n+10)}%`],options=[...new Set(choices)].slice(0,3);return baseQuestion('백분율로 나타내기',`100칸 중 ${n}칸을 색칠했습니다. 백분율은?`,percentGrid(n),options,ans,'100개 중 몇 개인지를 %로 나타냅니다.',`${n}/100=${ans}입니다.`);}
...
function qUnits(step){const amount=rand(2,9);if(step%2===1){const grams=amount*1000;return baseQuestion('무게 단위 바꾸기',`${grams}g은 몇 kg인가요?`,`<div class="unit-card">${grams} g = ? kg</div>`,nums(amount),amount,'1000g이 1kg입니다.',`${grams}g=${amount}kg입니다.`);}const ml=amount*1000;return baseQuestion('들이 단위 바꾸기',`${ml}mL는 몇 L인가요?`,`<div class="unit-card">${ml} mL = ? L</div>`,nums(amount),amount,'1000mL가 1L입니다.',`${ml}mL=${amount}L입니다.`);}
...
function qArea(step){const h=rand(2,7);if(step%3===1){const base=rand(2,8)*2,ans=base*h/2;return baseQuestion('삼각형 넓이',`밑변 ${base}cm, 높이 ${h}cm인 삼각형의 넓이는?`,`<div class="triangle-icon">△</div>`,nums(ans,10),ans,'밑변×높이÷2를 계산하세요.',`${base}×${h}÷2=${ans}cm²입니다.`);}if(step%3===2){const base=rand(3,9),ans=base*h;return baseQuestion('평행사변형 넓이',`밑변 ${base}cm, 높이 ${h}cm인 평행사변형의 넓이는?`,`<div class="quad-icon">▱</div>`,nums(ans,10),ans,'밑변×높이를 계산하세요.',`${base}×${h}=${ans}cm²입니다.`);}const w=rand(2,8),ans=w*h;return baseQuestion('직사각형 넓이',`가로 ${w}cm, 세로 ${h}cm인 직사각형의 넓이는?`,gridRect(w,h),nums(ans,8),ans,'가로×세로를 계산하세요.',`${w}×${h}=${ans}cm²입니다.`);}
function qCircle(step){const r=rand(2,6);if(step%2===1){const area=Math.round(3.14*r*r*100)/100,choices=[area,area+3.14,Math.max(.01,area-3.14),area+6.28].map(value=>Math.round(value*100)/100);return baseQuestion('원의 넓이',`반지름이 ${r}cm인 원의 넓이는? (원주율 3.14)`,`<div class="circle-visual" style="width:${r*22}px;height:${r*22}px"><i></i></div>`,choices.map(value=>({value:String(value),label:`${value}cm²`})),String(area),'반지름×반지름×원주율을 사용하세요.',`${r}×${r}×3.14=${area}cm²입니다.`);}const c=Math.round(2*3.14*r*100)/100,choices=[c,3.14*r,r*r,c+3.14].map(value=>Math.round(value*100)/100);return baseQuestion('원의 둘레',`반지름이 ${r}cm인 원의 원주는? (원주율 3.14)`,`<div class="circle-visual" style="width:${r*22}px;height:${r*22}px"><i></i></div>`,choices.map(value=>({value:String(value),label:`${value}cm`})),String(c),'2×반지름×원주율을 사용하세요.',`2×${r}×3.14=${c}cm입니다.`);}
...
function qVolume(step){const l=rand(2,5),w=rand(2,4),h=rand(2,4);if(step%2===1){const ans=2*(l*w+w*h+l*h);return baseQuestion('직육면체 겉넓이',`가로 ${l}cm, 세로 ${w}cm, 높이 ${h}cm인 직육면체의 겉넓이는?`,cubeStack(l,w,h),nums(ans,16),ans,'마주 보는 면이 두 개씩 있으므로 (가로×세로+세로×높이+가로×높이)×2를 계산하세요.',`(${l}×${w}+${w}×${h}+${l}×${h})×2=${ans}cm²입니다.`);}const ans=l*w*h;return baseQuestion('직육면체 부피',`가로 ${l}cm, 세로 ${w}cm, 높이 ${h}cm인 직육면체의 부피는?`,cubeStack(l,w,h),nums(ans,10),ans,'한 층의 개수에 층 수를 곱하세요.',`${l}×${w}×${h}=${ans}cm³입니다.`);}
...
function qTable(){const cats=rand(2,6);let dogs=rand(2,6);while(dogs===cats)dogs=rand(2,6);const ans=Math.max(cats,dogs);return baseQuestion('표 읽기','더 많은 동물의 수는 몇 마리인가요?',`<table class="data-table"><tr><th>고양이</th><th>강아지</th></tr><tr><td>${cats}</td><td>${dogs}</td></tr></table>`,nums(ans),ans,'표의 두 수를 비교하세요.',`더 많은 쪽은 ${ans}마리입니다.`);}
...
function qBarGraph(){let vals,max;do{vals=[rand(2,8),rand(2,8),rand(2,8)];max=Math.max(...vals);}while(vals.filter(value=>value===max).length!==1);const idx=vals.indexOf(max),labels=['가','나','다'];return baseQuestion('막대그래프 해석','가장 큰 값을 가진 항목은?',barVisual(vals,labels),labels.map((x,i)=>({value:String(i),label:x})),String(idx),'가장 긴 막대를 찾으세요.',`${labels[idx]}가 가장 큽니다.`);}
function qLineGraph(){let vals,trend;do{vals=[rand(2,6)];for(let i=1;i<4;i++)vals.push(Math.max(1,vals[i-1]+rand(-2,2)));const changes=vals.slice(1).map((value,index)=>Math.sign(value-vals[index]));trend=changes.every(change=>change===0)?'steady':changes.every(change=>change>=0)&&changes.some(change=>change>0)?'up':changes.every(change=>change<=0)&&changes.some(change=>change<0)?'down':'mixed';}while(trend==='steady');const explain={up:'줄어드는 구간 없이 증가하거나 같았습니다.',down:'늘어나는 구간 없이 감소하거나 같았습니다.',mixed:'증가한 구간과 감소한 구간이 모두 있습니다.'}[trend];return baseQuestion('꺾은선그래프 해석','각 구간의 변화를 모두 살펴보면 어떤 모습인가요?',lineVisual(vals),[{value:'up',label:'계속 증가하거나 같았다'},{value:'down',label:'계속 감소하거나 같았다'},{value:'mixed',label:'증가와 감소가 모두 있었다'}],trend,'첫 점과 마지막 점만 보지 말고 이웃한 점을 차례로 비교하세요.',explain);}
...
function qRatioGraph(){const a=[20,25,40,50,60][rand(0,4)],b=100-a,answer=`${a}%`,choices=[answer,`${b}%`,'100%',`${Math.min(90,a+10)}%`],options=[...new Set(choices)].slice(0,3);return baseQuestion('비율 그래프 읽기',`전체를 100으로 볼 때 파란 부분은 몇 %인가요? (파란색 ${a/5}칸, 전체 20칸)`,`<div class="ratio-strip" aria-label="20칸 중 파란색 ${a/5}칸"><span style="width:${a}%"></span><i style="width:${b}%"></i></div>`,options,answer,'파란 칸 수를 전체 20칸으로 나눈 뒤 100을 곱하세요.',`${a/5}÷20×100=${a}%입니다.`);}
...
window.auditAiedueMathQuestionQuality=function auditAiedueMathQuestionQuality(iterations=20){
  const issues=[];
  const samples=[];
  for(const item of allLessons()){
    for(let step=0;step<STEPS.length;step+=1){
      for(let sample=0;sample<iterations;sample+=1){
        try{
          const question=makeQuestion(item.type,step);
          const values=question.options.map(option=>String(option.value));
          const answer=String(question.answer);
          const context=`${item.id}/${STEPS[step]}/${sample+1}`;
          if(values.length<2)issues.push(`${context}: 선택지가 2개보다 적습니다.`);
          if(!values.includes(answer))issues.push(`${context}: 정답이 선택지에 없습니다 (${answer}).`);
          if(new Set(values).size!==values.length)issues.push(`${context}: 선택지가 중복됩니다.`);
          if([question.title,question.prompt,question.hint,question.explain,...question.options.map(option=>option.label)].some(value=>/undefined|NaN|null/.test(String(value))))issues.push(`${context}: 문항에 잘못된 값이 표시됩니다.`);
          if(/^원의 (넓이|둘레)$/.test(question.title)&&values.length!==4)issues.push(`${context}: 원 문항 선택지는 4개여야 합니다.`);
          if(question.title==='자연수와 분수의 곱셈'){
            const [numerator,denominator]=answer.split('/').map(Number);
            const filledCount=(question.visual.match(/class="filled"/g)||[]).length;
            if(numerator>=denominator||filledCount!==numerator)issues.push(`${context}: 분수 곱셈 그림과 정답이 일치하지 않습니다.`);
          }
          samples.push({nodeId:item.id,step,answer,optionCount:values.length});
        }catch(error){issues.push(`${item.id}/${STEPS[step]}/${sample+1}: ${error.message}`);}
      }
    }
  }
  return{lessonCount:allLessons().length,sampleCount:samples.length,issues,samples};
};
function installEntryPoints(){const hasDomainDashboard=Boolean(document.querySelector('#dashboard-section [data-math-domain]'));if(hasDomainDashboard)return;const dashboard=document.querySelector('#dashboard-section .math-stage-board');if(dashboard&&!dashboard.querySelector('[data-spiral-entry]')){const button=document.createElement('button');button.type='button';button.className='dashboard-stage-card level-card';button.dataset.spiralEntry='true';button.onclick=openSpiralMap;button.innerHTML='<span class="stage-badge">CURRICULUM</span><span class="stage-emoji">🌀</span><span class="level-title">나선형 배움</span><span class="level-desc">초등 1~6학년 수학</span><span class="stage-note">4개 영역 · 3개 학년군 · 개념별 성장 지도</span>';dashboard.appendChild(button);}const tray=document.getElementById('rpg-action-tray');if(tray&&!tray.querySelector('[data-spiral-entry]')){const button=document.createElement('button');button.className='rpg-action-button rpg-math-button';button.dataset.spiralEntry='true';button.onclick=openSpiralMap;button.innerHTML='<span class="rpg-action-icon">🌀</span><span>나선형 배움</span>';tray.appendChild(button);}}
