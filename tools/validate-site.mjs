...
const requiredFiles = [
    ...
    'teacher-tutorial.js',
    'teacher-tutorial-core.mjs',
    'teacher-tutorial.css',
    'stage-tutorial.js',
    'stage-tutorial-core.mjs',
    'stage-tutorial.css',
    ...
    'style.css',
    'firebase-config.js',
    'classroom-refresh.css',
    'classroom-tools.js',
    'classroom-tools-core.mjs',
    'classroom-service.js',
    'classroom-print.html',
    'aiedu-loading.js',
    ...['stage-1', 'stage-2', 'stage-3', 'stage-4', 'loading-painter', 'loading-writer', 'loading-reader'].map(name => `assets/classroom/${name}.webp`),
    'assets/classroom/stage-select.png',
    ...[100, 500, 1000, 5000, 10000, 50000].map(value => `assets/classroom/currency-${value}.png`),
    'korean-learning-records.mjs',
    'drawing-shape-catalog.mjs',
    'shape-zoo-core.mjs',
    'shape-zoo-game.js',
    'shape-zoo.css',
    'shape-zoo.html',
    'assets/shape-zoo-scenes.png',
...
assert(index.includes('class="rpg-profile-portrait" onclick="toggleRpgHudPanel(this)" aria-label="메뉴 펼치기"'), '원형 경험치가 메뉴 펼치기 기능과 연결되지 않았습니다.');
...
const literacyPrompt = section(app, 'function generateLiteracyPrompt', 'function generateLiteracyDetectivePrompt');
const literacyDetectivePrompt = section(app, 'function generateLiteracyDetectivePrompt', 'function parseAiQuestionResponse');
...
assert(!literacyPrompt.includes('추리형 독해 사건') && literacyPrompt.includes('흥미롭고 유익한 읽기 지문'), '일반 문해력 생성 프롬프트가 탐정단 추리 프롬프트와 분리되지 않았습니다.');
assert(literacyDetectivePrompt.includes('추리 결론과 핵심 근거 하나를 쉬운 한 문장') && literacyDetectivePrompt.includes('결론과 근거를 한두 문장'), '탐정단 easy/normal 서술형이 저난도 추리 결론+근거 답변으로 제한되지 않습니다.');
...
assert(!('acceptableAnswers' in normalizedLegacyQuestion), '데이터 서버 비허용 필드 acceptableAnswers가 공용 오답 문서에 포함됩니다.');
