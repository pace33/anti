...
function normalizeCurricularStepStats(raw = {}) {
    return {
        ...
        corrects: Math.max(0, Number(raw?.corrects || 0)),
        wrongs: Math.max(0, Number(raw?.wrongs || Math.max(0, Number(raw?.attempts || 0) - Number(raw?.corrects || 0))))
    };
}
function normalizeCurricularWriting(raw = {}) {
    ...
    Object.entries(wordStats).forEach(([key, value]) => {
        ...
        normalizedWordStats[word] = {
            word,
            step1: normalizeCurricularStepStats(value?.step1),
            step2: normalizeCurricularStepStats(value?.step2),
            step3: normalizeCurricularStepStats(value?.step3),
            lastTriedAt: value?.lastTriedAt || null,
            savedAt: value?.savedAt || new Date().toISOString()
        };
    });
    return {
        activeWords: Array.from(new Set((Array.isArray(raw?.activeWords) ? raw.activeWords : []).map(cleanKoreanWord).filter(isLikelyKoreanNounBankWord))).slice(0, 10),
...
function getCurricularWritingState() {
    dictationPortfolio = normalizeDictationPortfolio(dictationPortfolio);
    return dictationPortfolio.curricularWriting;
}
function getCurricularWords(fallback = []) {
    const state = getCurricularWritingState();
    const words = Array.from(new Set([...(state.activeWords || []), ...fallback].map(cleanKoreanWord).filter(isLikelyKoreanNounBankWord)));
...
function getLowestAccuracyReviewWords(limit = 3, excludeWords = []) {
    ...
    return Object.values(wordStats)
        .map((item) => {
            const word = cleanKoreanWord(item.word);
            const totals = [item.step1, item.step2, item.step3].reduce((acc, step = {}) => {
                acc.attempts += Math.max(0, Number(step.attempts || 0));
                acc.corrects += Math.max(0, Number(step.corrects || 0));
                return acc;
            }, { attempts: 0, corrects: 0 });
            return { word, attempts: totals.attempts, accuracy: totals.attempts ? totals.corrects / totals.attempts : 1, wrongs: Math.max(0, totals.attempts - totals.corrects) };
        })
        .filter((item) => item.word && item.attempts > 0 && item.wrongs > 0 && !exclude.has(item.word))
        .sort((a, b) => (a.accuracy - b.accuracy) || (b.wrongs - a.wrongs) || a.word.localeCompare(b.word, 'ko'))
        .slice(0, limit)
        .map((item) => item.word);
}
function prepareCurricularWritingRound(candidateWords = [], newWords = []) {
    const state = getCurricularWritingState();
    const cleanCandidates = Array.from(new Set(candidateWords.map(cleanKoreanWord).filter(isLikelyKoreanNounBankWord))).slice(0, 20);
    const hasPriorRound = state.totalRounds > 0 || state.history.length > 0 || Object.values(state.wordStats || {}).some((item) => ['step1','step2','step3'].some((key) => Number(item?.[key]?.attempts || 0) > 0));
    const photoLimit = hasPriorRound ? 7 : 10;
    const photoWords = cleanCandidates.slice(0, photoLimit);
    const reviewWords = hasPriorRound ? getLowestAccuracyReviewWords(3, photoWords) : [];
    const filler = Array.from(new Set([...cleanCandidates, ...newWords, ...(dictationPortfolio.koreanBank?.words || [])].map(cleanKoreanWord).filter(isLikelyKoreanNounBankWord)))
        .filter((word) => !photoWords.includes(word) && !reviewWords.includes(word));
    const activeWords = Array.from(new Set([...photoWords, ...reviewWords, ...filler])).slice(0, 10);
    const activeRoundId = `curricular-photo-${Date.now()}`;
    const photoCapturedAt = new Date().toISOString();
    dictationPortfolio.curricularWriting = normalizeCurricularWriting({
        ...state,
        activeWords,
...
function makeCurricularSentence(word, index = 0) {
    const clean = cleanKoreanWord(word) || '단어';
    const frames = [
        `오늘 배운 ${clean}을 바르게 설명해요.`,
        `교과서에서 ${clean}이라는 말을 찾았어요.`,
        `친구와 함께 ${clean}에 대해 이야기해요.`,
        `선생님이 ${clean}의 뜻을 알려 주셨어요.`,
        `나는 공책에 ${clean}을 또박또박 적어요.`,
        `${clean}은 오늘 수업에서 중요한 단어예요.`,
        `우리는 ${clean}을 넣어 문장을 만들어요.`,
        `${clean}을 읽고 뜻을 생각해 보아요.`,
        `칠판에 쓰인 ${clean}을 소리 내어 읽어요.`,
        `오늘의 노트에 ${clean}을 정리했어요.`
    ];
    return frames[index % frames.length];
}
...
function createCurricularDifficultySession(difficulty = getCurrentCurricularDan()) {
    ...
    return { kind: 'mission', mode: 'curricular', marker: CURRICULAR_AI_CANVAS_GRADING_MARKER, difficulty: dan, items, currentIndex: 0, graded: null, saved: false, autoSaved: false, startedAt: new Date().toISOString() };
}
...
function getCurricularMissionGradeResult(item = {}, index = 0) {
    return item.aiResult || {
        ...
        correct: item.correct === true,
        analysis: item.analysis || `${index + 1}번은 아직 AI 채점 전이에요.`
    };
}
function renderCurricularWritingCanvasCard(item, index, { compact = false } = {}) {
    const isMission = activeDictationSession?.kind === 'mission' && activeDictationSession?.mode === 'curricular';
    ...
    return `<div class="rounded-3xl bg-white border-2 ${item.retryMode ? 'border-orange-300 bg-orange-50/70 shadow-lg shadow-orange-100' : (item.aiGraded || item.traceComplete ? 'border-green-200 bg-green-50/60' : (active ? 'border-red-300 shadow-lg shadow-red-100' : 'border-red-100'))} p-5" data-curricular-card="${index}">
        ...
        <div class="mt-2 flex items-center justify-between gap-2 text-sm font-black">
            <span id="curricular-trace-status-${index}" class="${item.aiGraded || item.traceComplete ? (item.aiResult?.correct === false ? 'text-yellow-600' : 'text-green-600') : 'text-gray-400'}">${status}</span>
...
async function gradeCurricularCanvasItemWithAi(index) {
    ...
    const status = document.getElementById(`curricular-trace-status-${index}`);
    const original = button?.innerText || '확인';
    item.aiGrading = true;
...
function gradeCurricularCanvasSession() {
    ...
    return items.map((item, index) => getCurricularMissionGradeResult(item, index));
}
function configureDictationWorkspace(session, options = {}) {
    ...
    } else if (session.kind === 'trace' || isCurricularCanvasMission) {
    ...
    const hideSidePhotoCard = session.kind === 'trace' || isCurricularCanvasMission;
    const photoCard = document.getElementById('dictation-photo-card'); if (photoCard) photoCard.classList.toggle('hidden', hideSidePhotoCard);
    ...
    document.getElementById('dictation-main-instruction').innerText = session.kind === 'bank-camera' ? '책이나 공책을 카메라에 비춰 단어를 모아요.' : (session.kind === 'trace' ? '회색 글자를 화면에서 직접 따라 채워요.' : (isCurricularCanvasMission ? '듣기 버튼으로 문제를 듣고 빈 칸에 받아써요.' : (difficulty === 1 ? '듣기 버튼으로 단어를 듣고 빈 칸에 써요.' : (difficulty === 2 ? '문장 힌트를 보고 빈칸에 들어갈 단어를 써요.' : '문장을 듣고 전체 문장을 써요.'))));
    document.getElementById('dictation-sub-instruction').innerText = session.kind === 'bank-camera' ? 'OCR + AI 사진 분석 후 AI가 단어만 골라 단어 은행에 넣어요.' : (session.kind === 'trace' ? '확인을 눌러 80% 이상 채우면 체크가 떠요. 모두 완료하면 다음으로 넘어가요.' : (session.kind === 'practice' ? '연습에는 사진 촬영 채점이 필요 없어요.' : '확인 버튼으로 문항별 AI 사진 채점을 완료하고, 아래 이전/다음으로 이동해요.'));
    ...
    if (promptCard) promptCard.classList.toggle('hidden', session.kind === 'trace' || isCurricularCanvasMission);
    ...
    const saveBtn = document.getElementById('dictation-save-btn');
    const nextBtn = document.getElementById('dictation-next-btn');
    const bankBtn = document.getElementById('dictation-bank-save-btn');
    const gradeBtn = document.getElementById('dictation-grade-btn');
    const prevBtn = document.getElementById('dictation-prev-btn');
    const completeBtn = document.getElementById('dictation-complete-btn');
    answerBox.classList.add('hidden'); answerText.innerHTML = '';
    bankBtn.classList.add('hidden');
    gradeBtn.classList.toggle('hidden', session.kind !== 'mission' || isCurricularCanvasMission);
    saveBtn.classList.toggle('hidden', session.kind !== 'trace');
    saveBtn.innerText = '다음';
    nextBtn.classList.add('hidden');
    if (prevBtn) prevBtn.classList.add('hidden');
    if (completeBtn) completeBtn.classList.add('hidden');
    const photoHint = document.getElementById('dictation-photo-hint');
    if (photoHint) {
        photoHint.classList.toggle('hidden', hideSidePhotoCard);
        photoHint.innerText = session.kind === 'bank-camera' ? '카메라로 촬영하거나 파일을 선택해 단어를 추출하세요.' : (session.kind === 'mission' ? '태블릿 카메라 화면을 맞추고, 다 쓴 노트를 촬영하거나 파일을 선택하세요.' : '연습 모드에서는 공책 사진 탭을 사용하지 않아요.');
    }
...
function renderCurricularWordBankStats() {
    const state = getCurricularWritingState();
    const words = dictationPortfolio.koreanBank?.words || [];
    return words.map((word) => {
        const stat = state.wordStats?.[word] || {};
        const chips = [1, 2, 3].map((num) => {
            const s = stat[`step${num}`] || {};
            const rate = s.attempts ? Math.round((Number(s.corrects || 0) / Number(s.attempts || 1)) * 100) : 0;
            return `<span class="px-2 py-1 rounded-full bg-white text-red-500 border border-red-100">${num}단계 ${s.attempts ? `${rate}%` : '-'}</span>`;
        }).join(' ');
...
function renderMyDictationSection() {
    ...
    root.innerHTML = missions.length ? missions.map((record, index) => {
        ...
        const preview = items.slice(0, 5).map((item, i) => `<li>${i + 1}. ${escapeHtml(item.answer || item.sentence || '')} — ${item.correct ? '정답' : '오답'}${item.written ? ` / 쓴 답: ${escapeHtml(item.written)}` : ''}</li>`).join('');
...
window.startCurricularTraceStep = function startCurricularTraceStep(roundStart = null) {
    ...
    const words = Array.from(new Set((prepared?.words || []).map(cleanKoreanWord).filter(isLikelyKoreanNounBankWord))).slice(0, 10);
    if (!prepared?.ready || !words.length) {
        pendingCurricularWritingRoundStart = null;
        openDictationBankCamera({ afterSave: 'curricular-writing' });
        showAiedueAutoToast('사진부터 찍어주세요', '교과 맞춤쓰기는 새 노트 사진 분석이 끝난 뒤 2스텝이 시작돼요.', 3600);
        return;
    }
    pendingCurricularWritingRoundStart = null;
    ...
    configureDictationWorkspace({
        ...
        fromPhoto: true,
        curricularRoundId: prepared.roundId || state.activeRoundId || '',
        photoCapturedAt: prepared.photoCapturedAt || state.photoCapturedAt || '',
        photoWords: prepared.photoWords || state.photoWords || [],
        reviewWords: prepared.reviewWords || state.reviewWords || [],
        items: words.map((word) => ({ word, sentence: word, answer: word, canvasGuide: word, source: 'curricular-trace', coverage: 0, traceComplete: false })),
        currentIndex: 0,
        graded: null,
        saved: false,
        startedAt: new Date().toISOString()
    }, { badge: '2스텝 단어 따라쓰기', title: '화면 글자 따라 채우기', desc: `방금 찍은 사진에서 고른 단어 ${words.length}개를 회색 글자 위에 따라 채운 뒤 3스텝 받아쓰기로 넘어가요.` });
}
...
function resetWordBankCameraModal() {
    ...
    if (title) title.innerText = '📷 오늘의 노트 사진 찍기';
}
...
window.openDictationBankCamera = function openDictationBankCamera(options = {}) {
    const modal = document.getElementById('word-bank-camera-modal');
    if (!modal) {
    ...
    if (pendingWordBankCameraAfterSave === 'curricular-writing') {
        ...
        if (typeof window.goDictationDashboard === 'function') window.goDictationDashboard();
    }
    const title = modal.querySelector?.('h3');
    if (title && pendingWordBankCameraAfterSave === 'curricular-writing') title.innerText = '📷 교과 맞춤쓰기 노트 사진';
    modal.classList.remove('hidden');
...
async function gradeDictationSessionWithAi() {
    ...
    const mime = header.match(/data:(.*?);base64/)?.[1] || 'image/jpeg';
    const targets = (activeDictationSession?.items || []).map((item, index) => ({ index: index + 1, prompt: item.sentence, answer: item.answer || item.sentence, word: item.word || '', difficulty: Number(item.difficulty || activeDictationSession?.difficulty || 1) }));
    const prompt = `초등학생 교과 맞춤쓰기 공책 사진을 AI 시각 분석으로 채점하세요. OCR 텍스트 추출만 하지 말고, 이미지 속 학생 손글씨를 문제 문항별 정답과 직접 비교하세요.
- 1단계: 정답 단어가 공책에 있으면 correct=true
- 2단계: 문장 빈칸에 들어갈 정답 단어만 맞으면 correct=true
- 3단계: 정답 문장 전체가 정확히 쓰였으면 correct=true
학생이 실제로 쓴 것으로 보이는 답을 written에 적고, analysis에는 1문장으로 근거를 적으세요.
...
function applyCurricularWritingStats(graded, difficulty, now) {
    const state = getCurricularWritingState();
    const stepKey = `step${Math.max(1, Math.min(3, Number(difficulty || 1)))}`;
    ...
    graded.forEach((item) => {
        const word = cleanKoreanWord(item.word || item.answer || item.sentence);
        if (!word) return;
...
window.revealDictationAnswer = async function() {
    ...
    if (isCanvasMission) {
        window.completeCurricularMission();
        return;
    }
    const preview = document.getElementById('dictation-photo-preview');
...
window.previousDictationMissionProblem = function() {
...
window.completeCurricularMission = async function() {
    if (!(activeDictationSession?.kind === 'mission' && activeDictationSession.mode === 'curricular')) return;
    if (!activeDictationSession.items.every((item) => item.aiGraded)) {
        showModal('1번부터 10번까지 모두 확인 버튼으로 AI 채점을 완료한 뒤 완료할 수 있어요.');
        return;
    }
    ...
    if (completeBtn) completeBtn.classList.add('hidden');
    const nextBtn = document.getElementById('dictation-next-btn');
    if (nextBtn) { nextBtn.classList.remove('hidden'); nextBtn.disabled = false; nextBtn.innerText = '새 미션'; }
}
...
const LESSON_BATCHIM_INTRO_STROKES = {
    ...
    'ㄹ': [
        ...
        { start: [0.34, 0.46], end: [0.76, 0.46], direction: '오른쪽으로' },
            ...
            <div class="lesson27-family-cards">
                ${[
                    ...
                    <article class="lesson27-family-card state-idle" data-family-card="${item.key}" data-animation-state="idle"
                        role="button" tabindex="0" aria-label="${item.name} 손동작과 소리 듣기. 카드 전체를 눌러요."
                        onclick="activateLesson27FamilyCard(event, '${item.key}')"
                        onkeydown="activateLesson27FamilyCard(event, '${item.key}')">
...
function renderLesson27BatchimWritingPage() {
    const items = [
        { word:'숲', picture:'🌳', label:'숲 그림' },
        { word:'집', picture:'🏠', label:'집 그림' },
        { word:'앞', picture:'🧍↓', label:'앞을 나타내는 그림' },
        { word:'접시', picture:'🍽️', label:'접시 그림' },
        { word:'무릎', picture:'🦵', label:'무릎 그림' },
        { word:'덮밥', picture:'🍛', label:'덮밥 그림' },
        { word:'높다', picture:'🏢↕', label:'높다 그림' }
    ];
    ...
    return `<section class="lesson27-writing-page"><div class="lesson27-writing-heading"><strong>연습하기</strong><span>들리는 대로 받침쓰기</span></div><p class="lesson27-writing-instruction">낱말을 듣고 빈 받침 칸에 알맞은 <b>ㅂ 또는 ㅍ</b>을 직접 써 보세요.</p><div class="lesson27-writing-grid">${items.map((item, index) => `<article class="lesson27-writing-card"><button type="button" class="lesson27-picture-button" onclick="speakTextKo('${item.word}')" aria-label="${item.word} 소리 듣기"><span aria-hidden="true">${item.picture}</span><small>🔊 ${item.word}</small></button><div class="lesson27-tile-word" aria-label="${item.word}의 ㅂ 또는 ㅍ 받침 쓰기">${tiles(item.word, index)}</div></article>`).join('')}</div><div class="lesson27-writing-promise"><strong>약속하기</strong><span>‘ㅍ’과 ‘ㅂ’은 글자는 다르지만 받침소리는 <b>/ㅂ/</b>으로 같아요.</span></div></section>`;
}
...
const REPRESENTATIVE_FAMILY_CONFIGS = {
    ...
    29: {
        title:'ㄷ 받침가족', representative:'ㄷ', sound:'[ㄷ]', spokenSound:'드', playbackMode:'nameThenSound', family:[['ㄷ','디귿'],['ㅅ','시옷'],['ㅆ','쌍시옷'],['ㅈ','지읒'],['ㅊ','치읓'],['ㅌ','티읕'],['ㅎ','히읗']],
        writing:[['꽃','꼳','🌸'],['빗','빋','🪮'],['윷','윧','<img class="word-picture-asset yut-sticks-picture-asset" src="traditional-yut-sticks.webp" alt="">'],['옷','옫','🧥'],['솥','솓','🍲'],['낮','낟','☀️'],['씻다','씯다','🧼'],['젖소','젇소','🐄'],['벚꽃','벋꼳','🌸'],['찧다','찓다','🥣']],
...
const lesson30QuizState = { index:0, score:0, locked:false };
function renderLesson30Quiz() {
    lesson30QuizState.index=0; lesson30QuizState.score=0; lesson30QuizState.locked=false;
    const [choices,answer]=LESSON30_QUESTIONS[0];
    return `<section class="lesson30-solo-game" data-lesson30-quiz data-answer="${answer}"><div class="lesson30-game-head"><strong>👑 도전, 받침왕!</strong><span>혼자 듣고 알맞은 글자를 찾아 보세요.</span></div><div class="lesson30-game-score"><span>문제 <b data-lesson30-number>1</b> / 20</span><span>⭐ <b data-lesson30-score>0</b>점</span></div><div class="lesson30-progress-track"><i data-lesson30-progress></i></div><div class="lesson30-game-arena"><button type="button" class="lesson30-sound-button" onclick="playLesson30QuizSound()">🔊 문제 소리 듣기</button><p data-lesson30-feedback>소리를 잘 듣고 정답을 골라요.</p><div class="lesson30-choice-pair">${choices.map(choice=>`<button type="button" onclick="selectLesson30Answer(this,'${choice}')">${choice}</button>`).join('')}</div></div></section>`;
}
window.playLesson30QuizSound=function playLesson30QuizSound(){const question=LESSON30_QUESTIONS[lesson30QuizState.index];if(question)speakTextKo(question[1]);};
function updateLesson30Quiz(){const page=document.querySelector('[data-lesson30-quiz]');if(!page)return;if(lesson30QuizState.index>=LESSON30_QUESTIONS.length){page.classList.add('is-complete');page.querySelector('.lesson30-game-arena').innerHTML=`<div class="lesson30-crown">👑</div><strong class="lesson30-finish-title">받침왕이 되었어요!</strong><p>20문제를 모두 해결했어요. 최종 점수 ${lesson30QuizState.score}점!</p>`;page.querySelector('[data-lesson30-number]').textContent='20';page.querySelector('[data-lesson30-score]').textContent=lesson30QuizState.score;page.querySelector('[data-lesson30-progress]').style.width='100%';return;}const [choices,answer]=LESSON30_QUESTIONS[lesson30QuizState.index];page.dataset.answer=answer;page.querySelector('[data-lesson30-number]').textContent=lesson30QuizState.index+1;page.querySelector('[data-lesson30-score]').textContent=lesson30QuizState.score;page.querySelector('[data-lesson30-progress]').style.width=`${lesson30QuizState.index/LESSON30_QUESTIONS.length*100}%`;page.querySelector('[data-lesson30-feedback]').textContent='소리를 잘 듣고 정답을 골라요.';const pair=page.querySelector('.lesson30-choice-pair');pair.innerHTML=choices.map(choice=>`<button type="button" onclick="selectLesson30Answer(this,'${choice}')">${choice}</button>`).join('');lesson30QuizState.locked=false;window.setTimeout(()=>playLesson30QuizSound(),250);}
window.selectLesson30Answer=function selectLesson30Answer(button,selected){if(lesson30QuizState.locked)return;const page=button.closest('[data-lesson30-quiz]'),feedback=page.querySelector('[data-lesson30-feedback]');if(selected!==page.dataset.answer){button.classList.add('is-wrong');feedback.textContent='아쉬워요. 다시 선택해 보아요.';speakTextKo('다시 선택해 보아요.');return;}lesson30QuizState.locked=true;lesson30QuizState.score+=10;button.classList.add('is-correct');page.querySelectorAll('.lesson30-choice-pair button').forEach(item=>item.disabled=true);feedback.textContent=`정답이에요! ${selected}!`;speakTextKo(`정답이에요. ${selected}.`);lesson30QuizState.index+=1;window.setTimeout(updateLesson30Quiz,950);};
...
function getUnit9WordWritingGuide(word) {
    ...
    return letters.length === 1 ? `${word}/${word}` : letters.join('/');
}
function getUnit9WordWritingColumnCount(word) {
    return Math.max(2, Array.from(word).length);
}
...
window.activateLesson27FamilyCard = function activateLesson27FamilyCard(event, kind) {
    if (!event || !kind) return;
    const isKeyboard = event.type === 'keydown';
    if (isKeyboard && (event.repeat || !['Enter', ' '].includes(event.key))) return;
    if (event.target instanceof Element && event.target.closest('button, a, input, select, textarea')) return;
    if (isKeyboard) event.preventDefault();
    window.playLesson27FamilyCard(kind);
};
...
function getTraceGridLayout(canvas, chars) {
    ...
    const cols = isSentenceLines
        ? 1
        : (configuredCols || (isMakeLetterGrid ? 5 : (isDoubleRow ? 5 : (isWordDoubleRow ? 2 : chars.length))));
    const rows = isSentenceLines
        ? Math.max(1, chars.length)
        : ((configuredCols || isMakeLetterGrid) ? Math.ceil(chars.length / cols) : (isDoubleRow || isWordDoubleRow ? 2 : 1));
    return { cols, rows };
}
