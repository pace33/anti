import { normalizeStudentNames, provisionStudentRows, CURRENCY_ASSETS, escapePrintHtml as esc, rosterMarkup, shopMarkup } from './classroom-tools-core.mjs';
...
export function installClassroomTools(service) {
    ...
    function render() {
        dialog.innerHTML = `<form method="dialog" class="classroom-dialog-content"><div class="classroom-dialog-heading"><h2 id="new-students-title">신규 학생 추가</h2><button type="button" data-close ${busy ? 'disabled' : ''} aria-label="닫기">✕</button></div><p>이름을 입력하면 로그인 번호가 만들어지고 우리 학급에 바로 추가됩니다. 같은 이름의 학생도 각각 가입할 수 있어요.</p><div class="classroom-name-list">${rows.map((row, i) => `<div class="classroom-name-row"><label for="new-student-${i}">이름 ${i + 1}</label><input id="new-student-${i}" data-name="${i}" maxlength="30" value="${esc(row.name)}" ${busy || row.code ? 'disabled' : ''} autocomplete="off"><button type="button" data-remove="${i}" ${busy || row.code || rows.length === 1 ? 'disabled' : ''} aria-label="이름 ${i + 1} 삭제">삭제</button><span class="classroom-row-status ${row.status === 'failed' ? 'failed' : ''}">${row.status === 'complete' ? `완료 · 로그인 번호 ${esc(row.code)}` : row.status === 'working' ? '가입 및 학급 연결 중…' : row.status === 'failed' ? `미완료 · ${esc(row.error)} (다시 시도 가능)` : ''}</span></div>`).join('')}</div><div class="classroom-dialog-footer"><button type="button" data-add ${busy || rows.length >= 40 ? 'disabled' : ''}>＋ 이름 추가</button><span role="status" aria-live="polite">${rows.filter(row => row.status === 'complete').length} / ${rows.length}명 완료</span><button type="button" class="classroom-primary" data-submit ${busy || rows.every(row => row.status === 'complete') ? 'disabled' : ''}>${busy ? '신규 추가 중…' : rows.some(row => row.status === 'failed') ? '미완료 학생 다시 추가' : '신규 추가'}</button></div></form>`;
    ...
    async function submit() {
        ...
        try {
            ...
            rows.filter(row => row.status !== 'complete').forEach(row => { row.status = 'failed'; row.error = error.message; });
        }
        finally {
